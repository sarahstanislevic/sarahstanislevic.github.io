function random_choice(num) {  // New function called random_choice that takes one parameter, num (or a number)
...
function mash_choice() {  // New function called mash_choice that doesn't take any parameters 
    ...
    var randomNum = random_choice(4);  // Use the above function to get a number between 0 and 4
...
function get_answer(category) { 
    ...
    return choices[random_choice(choices.length)];   // Pick and return a random choice choice.length = number of answers the user provided in that category 
}
